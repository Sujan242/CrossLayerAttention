...
class CustomDynamicCache(DynamicCache):
    ...
    def update(
        ...
        if self._seen_tokens > 1:
            ...
            prev_token_key_cache = torch.cat([self.key_cache[i][:, :, :self._seen_tokens-1,: ] for i in range(len(self.key_cache)) if i != layer_idx], dim=2)
            prev_token_value_cache = torch.cat([self.value_cache[i][:, :, :self._seen_tokens-1, ] for i in range(len(self.key_cache)) if i != layer_idx], dim=2)
